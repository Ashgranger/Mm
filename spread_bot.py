...
class SpreadBot:
    def __init__(self, client, api_client, dry_run: bool = False):
        ...
        if not dry_run:
            import lighter as _l
            self.order_api   = _l.OrderApi(api_client)
            self.account_api = _l.AccountApi(api_client)
        else:
            self.order_api   = None
